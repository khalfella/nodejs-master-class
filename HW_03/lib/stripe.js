...
stripe.captureTheCharge = function (charge_id, callback) {
	...
	var req = https.request(requestOptions, function (res) {
			...
			var rep = null;
                        try {
                                rep = JSON.parse(rawData);
                        } catch(e) {
                                callback(e.message);
                                return;
                        }

			if (rep.captured === true && rep.id == charge_id) {
				callback(null);
				return;
			}
			callback('Failed to capture the payment');
		});
...
stripe.verifyPayment = function (token_id, charge_id, cart_id, callback) {
	...
	var req = https.request(requestOptions, function (res) {
		...
		res.on('end', function() {
			...
			if (rep.id == charge_id && rep.description == cart_id) {
				callback(null);
				return;
			}

			callback('Failed to verify this charge');
		});
	});
